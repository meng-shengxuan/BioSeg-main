...
class MambaVisionMixer(nn.Module):
    def __init__(
        ...
        # ===== Conv1D branches (🔑 length-preserving) =====
        ...
        # ===== Gate (single complementary gate) =====
    ...
    def forward(self, hidden_states):
        """
        hidden_states: (B_eff, L_eff, D)
        return: (B_eff, L_eff, D)
        """

        # ===== Input projection =====
        xz = self.in_proj(hidden_states)          # (B_eff, L_eff, d_inner)
        xz = rearrange(xz, "b l d -> b d l")
        x, z = xz.chunk(2, dim=1)                 # (B_eff, d_inner//2, L_eff)

        # ===== Conv1D (length preserved) =====
        ...
        dt = rearrange(self.dt_proj(dt), "(b l) d -> b d l", l=L_eff)
        Bp = rearrange(Bp, "(b l) dstate -> b dstate l", l=L_eff).contiguous()
        Cp = rearrange(Cp, "(b l) dstate -> b dstate l", l=L_eff).contiguous()
        ...
        )   # (B_eff, d_inner//2, L_eff)

        # ===== Conv-only branch =====
        y_conv = z                               # (B_eff, d_inner//2, L_eff)

        # ===== Gated fusion =====
        gate = self.gate_act(
            self.gate_proj(rearrange(y_conv, "b d l -> b l d"))
        )
        gate = rearrange(gate, "b l d -> b d l")

        y = gate * y_ssm + (1.0 - gate) * y_conv
